...
const ContactPage: React.FC = () => {
  ...
  return (
    <div className="pt-32 pb-24 bg-[#1A1A1A] text-white min-h-screen relative overflow-hidden flex flex-col">
      {/* Background Decor - Fixed for better mobile performance */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-[#4A6741]/20 blur-[100px] rounded-full pointer-events-none"></div>
          ...
          <div className="lg:col-span-5 w-full">
            ...
            >
              <h1 className="text-6xl md:text-8xl lg:text-9xl serif font-bold mb-8 leading-tight">
                Let's <br/> <span className="italic text-[#4A6741]">Write</span> History.
              </h1>
  ...
  className="w-full sm:w-auto flex items-center justify-center space-x-3 bg-white text-black px-8 py-5 rounded-full font-bold hover:bg-[#4A6741] hover:text-white transition-all shadow-xl"
>
  <Download size={18} />
  <span>Get My Resume</span>
</a>
                <div className="flex space-x-4">
                  <a href="https://www.linkedin.com/in/ezenwa-blessing-937127353" className="p-5 bg-white/5 rounded-full hover:bg-white/10 transition-colors border border-white/10"><Linkedin size={20} /></a>
                  <a href="#" className="p-5 bg-white/5 rounded-full hover:bg-white/10 transition-colors border border-white/10"><Twitter size={20} /></a>
                </div>
              </div>
            </motion.div>
          </div>
              ...
              <form className="space-y-6 md:space-y-8" onSubmit={handleSubmit}>
                <div className="grid md:grid-cols-2 gap-6 md:gap-8">
                  <div className="space-y-2">
                    ...
                    <input type="text" name="name" required placeholder="Johnathan" className="w-full bg-[#FDFBF7] border-2 border-transparent focus:border-[#4A6741] rounded-[1.5rem] md:rounded-[2rem] px-8 py-4 md:py-5 outline-none transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400 ml-4">Email</label>
                    <input type="email" name="email" required placeholder="john@domain.com" className="w-full bg-[#FDFBF7] border-2 border-transparent focus:border-[#4A6741] rounded-[1.5rem] md:rounded-[2rem] px-8 py-4 md:py-5 outline-none transition-all" />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400 ml-4">Subject</label>
                  <select name="subject" className="w-full bg-[#FDFBF7] border-2 border-transparent focus:border-[#4A6741] rounded-[1.5rem] md:rounded-[2rem] px-8 py-4 md:py-5 outline-none transition-all appearance-none text-gray-600">
                ...
                <div className="space-y-2">
                  ...
                  <textarea name="message" required rows={4} placeholder="Describe the voice you want to project..." className="w-full bg-[#FDFBF7] border-2 border-transparent focus:border-[#4A6741] rounded-[1.5rem] md:rounded-[2rem] px-8 py-4 md:py-5 outline-none transition-all resize-none"></textarea>
                </div>

                <button type="submit" className="w-full group bg-[#1A1A1A] text-white py-5 md:py-6 rounded-full flex items-center justify-center space-x-4 hover:bg-[#4A6741] transition-all duration-500">
                  <span className="font-bold uppercase tracking-[0.3em] text-xs">Send Message</span>
                  <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
                </button>

                {status && (
                  <p className={`text-center text-sm font-bold mt-4 ${status.includes("successfully") ? "text-[#4A6741]" : "text-red-500"}`}>
                    {status}
                  </p>
                )}
              </form>
